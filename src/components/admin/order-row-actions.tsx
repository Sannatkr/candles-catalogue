...
import { deleteOrder, setOrderStatus } from "@/lib/admin/actions";
import { MANAGE_STATUSES, ORDER_STATUS_LABEL, type OrderStatus } from "@/lib/admin/order-status";
...
export function OrderRowActions({
  ...
  // A portal at fixed coordinates keeps it above everything.
  useLayoutEffect(() => {
    if (!open) return;
  ...
  const menu = open && coords && (
    ...
    >
      ...
      {MANAGE_STATUSES.filter((s) => s !== status).map((next) => (
      ...
      <div className="border-t border-line-soft">
        {confirming ? (
          <form action={deleteOrder} className="flex items-center gap-2 px-3.5 py-2.5">
            <input type="hidden" name="id" value={id} />
            <button type="submit" className="rounded-full bg-ember-deep px-3 py-1.5 text-[0.78rem] text-canvas">
              Delete for good
            </button>
            <button
              type="button"
