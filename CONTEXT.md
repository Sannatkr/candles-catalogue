...
- 2026-09-06 — **Site logo is the full circular seal, not the monogram — this supersedes the entry below it.** The seal as drawn is an *ellipse*, 5.3% taller than wide (fitted cx 623.2, cy 632.4, rx 591.7, ry 622.9), and the source frame clipped the bottom of the ring, so "just crop it" gives a lopsided oval with a flat spot. `scripts/make-logo.py` fits the ellipse, stretches x by ry/rx to make it a true circle, lays a perfect annulus underneath to fill the clipped arc, and masks to a circle — `public/logo.png`, 400px. Header 44px, footer 56px, admin 36px; `public/logo-mark.png` deleted. **The tab icons stay the bare swan monogram on purpose:** a 16px favicon carrying the ring *and* the wordmark is an unreadable smudge, and the ring costs the swan about a third of its width.

- 2026-09-06 — **The real brand logo is now on the site and in the browser tab.** The source was a JPEG of the purple swan seal on white, so the white was lifted to alpha (and edge pixels un-blended from white) to give `public/logo.png` — the full seal — and `public/logo-mark.png` — the crowned-swan monogram alone. **The header, footer and admin rail use the monogram, not the full seal:** the seal's own "SUGANDHA CANDLES" lettering turns to mush below ~120px and would have duplicated the wordmark sitting next to it. Tab icons follow Next's file conventions — `src/app/icon.png` (192px), `src/app/apple-icon.png` (180px, flattened onto cream because Apple composites transparency onto black) and a rebuilt `src/app/favicon.ico` (16/32/48) replacing the stock Next one Google had been showing. Also filled in the long-open `logo` field on the `Organization` JSON-LD, pointing at the full seal.
